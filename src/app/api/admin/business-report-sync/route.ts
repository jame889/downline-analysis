...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const [telegramNotified, jarvisPush] = await Promise.all([
      ...
      pushDownlineEventsToJarvis(jarvisEvents),
    ])

    if (!jarvisPush.ok) {
      console.warn('[business-report-sync] Jarvis push failed; fallback endpoint remains available', jarvisPush)
    }
    ...
    const status = {
      ...
      telegramNotified,
      supabaseSynced,
      jarvisEventCount: jarvisEvents.length,
      jarvisPushed: jarvisPush.ok && !jarvisPush.skipped,
      jarvisPushSkipped: Boolean(jarvisPush.skipped),
      jarvisPushStatus: jarvisPush.status ?? null,
    }
