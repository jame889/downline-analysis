...
import { formatPersonalityForCoach } from '@/lib/personality'
import { loadPersonalityProfile } from '@/lib/personality-storage'
...
async function loadKnowledge(): Promise<string> {
  ...
  if (USE_SUPABASE) {
    const res = await fetch(`${SUPABASE_URL}/storage/v1/object/list/${BUCKET}`, {
      method: 'POST', headers: sbHeaders(), body: JSON.stringify({ prefix: '', limit: 200 }),
    })
    if (res.ok) {
      ...
      for (const f of files.filter(f => f.name.endsWith('.json'))) {
        ...
        if (r.ok) { try { docs.push(await r.json()) } catch { /* skip */ } }
      }
    }
  } else {
    ...
    docs = fs.readdirSync(KNOWLEDGE_DIR)
      ...
      .map(f => { try { return JSON.parse(fs.readFileSync(path.join(KNOWLEDGE_DIR, f), 'utf-8')) } catch { return null } })
      .filter(Boolean)
  }
  const excerpts = docs.map(d => d.content ? `### ${d.title}\n${d.content.slice(0, 3000)}` : null).filter(Boolean) as string[]
  if (!excerpts.length) return ''
  return `\n\n=== ฐานความรู้ด้านธุรกิจเครือข่าย Binary ===\n${excerpts.join('\n\n---\n\n')}`
}

async function buildSystemPrompt(coachData: Record<string, unknown> | null, personalitySection: string): Promise<string> {
  if (!coachData) {
...
ใช้ Personality Profile เพื่อปรับรูปแบบการสื่อสารและแผนพัฒนาผู้นำเท่านั้น
ห้ามตัดสินศักยภาพ ความซื่อสัตย์ หรือตำแหน่งจากประเภทบุคลิกภาพ
ต้องใช้ข้อมูลพฤติกรรมจริง KPI และบริบทประกอบเสมอ${knowledge}`
  }

  const d = coachData as {
    month?: string
    member?: { id: string; name: string }
    balance?: { L: number; R: number; total: number; weakSide: string; weakVol: number; strongVol: number; weakPct: number; gapToBalance: number; urgency: string }
  ...
  const gen1Str = d.gen1?.map(g => `${g.name} (${g.id}): ${g.is_active ? 'Active' : 'Inactive'}, ลึก ${g.depth} ชั้น, ทีม ${g.sub_count} คน, active ${g.active_in_sub} คน, ${g.is_safe_zone ? 'SAFE ZONE' : 'ต้องขุดต่อ'}`).join('\n') ?? ''
  const newMemberStr = d.newMembers?.map(m => `${m.name} (${m.id}): ${m.is_tapped ? 'ขุดลึกแล้ว' : 'ยังไม่ได้ขุดลึก'}`).join('\n') ?? ''
  const knowledge = await loadKnowledge()
...
=== กติกาการใช้ Personality Profile ===
ใช้เพื่อปรับวิธีสื่อสาร การเรียนรู้ Feedback และแผนพัฒนาผู้นำเท่านั้น
ห้ามสรุปศักยภาพ ความซื่อสัตย์ ความสำเร็จ หรือตำแหน่งจากประเภทบุคลิกภาพ
ใช้คำว่า “มีแนวโน้ม” และพิจารณา KPI พฤติกรรมจริง และบริบทควบคู่เสมอ
คะแนน 45–55 หมายถึงสามารถปรับใช้ได้ทั้งสองด้าน

ตอบเป็นภาษาไทย สั้น กระชับ ตรงประเด็น ใช้ข้อมูลข้างต้นประกอบคำแนะนำเสมอ

=== การแสดง Chart ===
- [CHART:balance] เมื่อพูดถึงสมดุลซ้าย/ขวา
- [CHART:levels] เมื่อพูดถึง Active Rate ตามชั้น
- [CHART:safezone] เมื่อพูดถึง Safe Zone หรือสายงาน Gen1
- [CHART:newmembers] เมื่อพูดถึงสมาชิกใหม่หรือการขุดลึก
ใส่เฉพาะเมื่อ Chart ช่วยให้เข้าใจง่ายขึ้น${knowledge}`
}
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const profile = await loadPersonalityProfile(session.memberId).catch(() => null)
    const personalitySection = profile && profile.visibility !== 'private' ? formatPersonalityForCoach(profile) : ''
    const systemPrompt = await buildSystemPrompt(coachData, personalitySection)

    const ollamaRes = await fetch(`${OLLAMA_URL}/api/chat`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ model: MODEL, messages: [{ role: 'system', content: systemPrompt }, ...messages], stream: true }),
    })
    if (!ollamaRes.ok) return Response.json({ error: 'Ollama error' }, { status: 500 })
